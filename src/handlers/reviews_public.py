...
from stripe_link.repositories.documents import RepositoryError, products_repository, reviews_repository, sites_repository
...
def handler(event, context, *, reviews_repo=None, products_repo=None, sites_repo=None):
    ...
    if method == "POST":
        return submit_review(event, reviews_repo, products_repo)
...
def render_form(event, products_repo, sites_repo):
    ...
    return _html_response(_form_html(tenant_id, product_id, product_name, business, str(params.get("token") or "")))


def submit_review(event, reviews_repo, products_repo):
...
def _form_html(tenant_id, product_id, product_name, business, token, error: str = "") -> str:
    stars = "".join(
