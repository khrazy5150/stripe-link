...
def render_bnpl_messaging_scripts() -> list[str]:
    ...
    return [
        ...
        f"      render({amount});",
        "      var options = document.querySelector('.sl-price-options');",
        "      var cards = options ? options.querySelectorAll('.sl-price-option') : [];",
        "      // Primary signal: the price selector's own event bus (clicks set .checked programmatically,",
        "      // which never fires native 'change', so we can't rely on a change listener alone).",
        ...
        "          if (i >= 0 && cards[i]) render(cardAmount(cards[i]));",
        "        });",
        "      }",
        "      // Fallbacks for pages without the bus: direct click + keyboard-driven native change.",
        "      if (options) {",
        "        options.addEventListener('click', function(ev){",
        "          var card = ev.target && ev.target.closest && ev.target.closest('.sl-price-option');",
        "          if (card) render(cardAmount(card));",
        "        });",
        "        options.addEventListener('change', function(){",
        "          var checked = options.querySelector('input[type=\"radio\"]:checked');",
        "          var card = checked && checked.closest('.sl-price-option');",
        "          if (card) render(cardAmount(card));",
        "        });",
        "      }",
        "    } catch (e) {}",
