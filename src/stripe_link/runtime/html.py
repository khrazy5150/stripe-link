...
def render_page_interactions_script(page: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        "                if (serverCart && ln.line_id && serverEnabled) {",
        ...
        "                  rm.addEventListener('click', () => removeLine(ln.line_id)); row.appendChild(rm);",
        "                }",
        ...
        "            fetch(cartEndpoint + '?tenant_id=' + encodeURIComponent(tenantId) + '&cart_id=' + encodeURIComponent(existingId))",
        "              .then((r) => r.ok ? r.json() : Promise.reject(r)).then((d) => { if (d && d.cart) serverCart = d.cart; renderMinicart(); }).catch(renderMinicart);",
        "          } else { renderMinicart(); }",
        "        }",
        "        applyTarget(0);",
        "      }",
