...
def render_bnpl_messaging_scripts() -> list[str]:
    ...
    return [
        ...
        "      // The element renders in a Stripe iframe, so the page's CSS can't reach it; feed it the page's own",
        "      // colors (theme-adaptive: light text on a dark page, dark on a light one) via the appearance API.",
        "      var _root = getComputedStyle(document.documentElement);",
        "      var _appv = {};",
        "      var _txt = getComputedStyle(document.body).color; if (_txt) { _appv.colorText = _txt; _appv.colorTextSecondary = _txt; }",
        "      var _accent = (_root.getPropertyValue('--sl-theme-accent') || _root.getPropertyValue('--sl-cta-bg') || '').trim(); if (_accent) _appv.colorPrimary = _accent;",
