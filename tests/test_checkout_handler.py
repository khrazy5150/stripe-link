        ...
        "prices": [
            {
                ...
                "stripe_price_id": "price_stripe_bump",
                "currency": "usd",
                "unit_amount": 500,
                "quantity": 1,
                "context": "order_bump",
...
class CheckoutHandlerTests(unittest.TestCase):
    ...
    def _offer_with_bump(self):
        offer = dict(self.offer)
        offer["funnel"] = {"order_bumps": [{"product_id": "prod_insulated_thermos", "price_id": "price_bump_thermos"}]}
        return offer

    def _checkout_with_bump(self, offer, bump_product, *, stripe_repo=None):
        return handler(
            {
                "httpMethod": "GET",
                "queryStringParameters": {
    ...
    def test_checkout_emits_order_bump_as_optional_item(self):
        ...
        payload = parse_qs(self.requests[0].data.decode("utf-8"))
        # The bump rides Stripe optional_items by its SYNCED price, not as a mandatory line item.
    ...
    def test_checkout_skips_unsynced_order_bump(self):
        _bump_offer, bump_product = order_bump_fixtures()
        bump_product["prices"][0]["stripe_price_id"] = None  # not synced -> can't be an optional_item
        response = self._checkout_with_bump(self._offer_with_bump(), bump_product)

        payload = parse_qs(self.requests[0].data.decode("utf-8"))
        self.assertNotIn("optional_items[0][price]", payload)  # skipped, not force-added
        self.assertNotIn("metadata[order_bump_ids]", payload)  # empty value dropped by parse_qs
        self.assertEqual(response["statusCode"], 303)  # checkout still proceeds

    def test_checkout_fee_is_on_base_subtotal_not_optional_bump(self):
        _bump_offer, bump_product = order_bump_fixtures()
        with patch.dict(os.environ, {"STRIPE_SECRET_KEY": "sk_live_platform"}, clear=False):
            self._checkout_with_bump(self._offer_with_bump(), bump_product, stripe_repo=FakeConnectStripeKeysRepository())
