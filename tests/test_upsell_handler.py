...
class ProcessUpsellTests(unittest.TestCase):
    ...
    def test_charges_named_upsell_product_standalone_even_when_not_a_landing_item(self):
        # A real funnel offer: its landing item is a DIFFERENT product (not even loaded here). The upsell is
        # named on the request, so process_upsell charges that product@price standalone (P3.2c).
        funnel_offer = {
            "offer_id": "offer_funnel", "tenant_id": "tenant_demo", "status": "active",
            "product_intent": "transaction",
            "items": [{"product_id": "prod_landing", "price_id": "price_landing", "quantity": 1}],
            "funnel": {"upsells": [{"product_id": "prod_creatine_gummies", "price_id": "price_upsell_1bottle"}]},
        ...
        opener = FakeStripeOpener({
            ...
            ("POST", "/v1/payment_intents"): {"id": "pi_up", "status": "succeeded"},
        })
        response = handler(
            self.base_event(offer_id="offer_funnel", product_id="prod_creatine_gummies", price_id="price_upsell_1bottle", sequence=2),
            None,
            offers_repo=FakeRepository("offer_id", [funnel_offer]),
            products_repo=FakeRepository("product_id", [self.product]),  # only the upsell product; prod_landing absent
        ...
        self.assertEqual(response["statusCode"], 201)
        order = self.orders_repo.get("tenant_demo", "order_cs_test_123_upsell_2")
        self.assertEqual(order["amount_total"], 2700)  # the UPSELL price, not the landing product
