...
7. **Cleanup.** Once verified, retire `test.juniorbay.com` / `preview.juniorbay.com` or keep them as permanent
   aliases (cheap; avoids breaking any copied links).
...
host actually serves — so previews never point at a not-yet-live host. Rollback = revert the `app_config` host
values; the dashboard falls back to the prior hosts (`test.juniorbay.com` / `preview.juniorbay.com`), which remain as
aliases.
